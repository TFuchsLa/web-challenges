...
async function fetchUserData(url) {
  try {
    ...
    if (!response.ok) {
      // Fehler werfen mit Statuscode und Nachricht
      throw new Error(`Fehler: ${response.status} - Benutzer nicht gefunden.`);
    }

    if (!contentType || !contentType.includes("application/json")) {
      throw new Error(
        `Unerwarteter Inhaltstyp: '${contentType}' ist kein gültiges JSON.`
      );
    }
